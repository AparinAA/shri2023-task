...
function Main() {
  ...
  const onSelectInput = function (event) {
    setActiveTab(event.target.value);
  };
  let sizes = [];
  const onSize = function (size) {
    sizes.push(size);
  };
  React.useEffect(function () {
    const sumWidth = sizes.reduce(function (acc, item) {
      return acc + item.width;
    }, 0);
    const sumHeight = sizes.reduce(function (acc, item) {
